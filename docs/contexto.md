...
- Detalhes e schema do dialeto: `llm/README.md`. **Pendente:** rascunhar a v0 do system prompt (insumos prontos: `mvp_subset.json` inline + protocolo de feedback abaixo).
...
- ~~Subset curado do MVP para o system prompt~~ **feito**: `llm/build_mvp_subset.py` → `mvp_subset.json` (33 classes, validadas contra o catálogo).
...
- **Rascunhar a v0 do system prompt de geração de .nk** — insumos prontos (subset inline + protocolo de feedback); falta o texto do prompt em si.
