...
- **Aridade default É mensurável no GUI, mas pelo lado do PASTE, não do serializador** (método do `map_inputs_gui.py` v4): serializar o nó com `nodeCopy`, extrair o bloco e **remover a linha `inputs`**; montar snippet sintético com 6 Dots `inputs 0` (lastro empilhado) + o bloco; `nuke.nodePaste` e **contar os inputs conectados** do nó colado — esse número é o que o paste pop-a sem o knob, exatamente a pergunta que o `parseNK` responde. O script embute âncoras de verdade conhecida (Constant=0, Blur=1, Grade=1, Merge2=2, ChannelMerge=2, Keymix=3, Roto=1) e imprime PASS/FAIL — se falhar, não usar o JSON.
